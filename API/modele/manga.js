...
module.exports.patchManga = async (id, title, synopsis, new_price, type, sub_genre, author, publisher, main_picture, is_finish, client) => {
    return await client.query("UPDATE manga SET title = $1, synopsis = $2, new_price = $3, type = $4, sub_genre = $5, author = $6, publisher = $7, main_picture = $8, is_finish = $9 WHERE id = $10", [title, synopsis, new_price, type, sub_genre, author, publisher, main_picture, is_finish,id]);
}

module.exports.deleteManga = async (id, client) => {
    return await client.query("DELETE FROM manga WHERE id_manga = $1", [id]); /* transaction delete manga = delete all tome = et all table readetome */
}